...
class Akamai:
    ...
    def post_pixel(self, payload, pixel_url):
        ...
        try:
            ...
            while r is None:
                try:
                    r = self.session.post(url=pixel_url, data=payload, headers=headers)
                except Exception as e:
        ...
        try:
            ...
            while r is None:
                try:
                    ...
                    r = self.session.post(url=self.bm_ep, data=data, headers=headers)
                    print(r.text)
                except Exception as e:
            ...
            while r is None:
                try:
                    if self.prefetch_ep is not None:
                        ...
                        r = self.session.get(self.prefetch_ep, headers=headers)
                    ...
                    r = self.session.get(self.bm_ep, headers=headers)
                except Exception as e:
                    raise SystemExit
                    print(f'ERROR WHILE GETTING AKAMAI COOKIE: {e}')
            if self.session.cookies.cookieJar.get('_abck', None) is not None:
                self.abck = r.cookies.get('_abck')
                return True
        except Exception as e:
            raise SystemExit
