...
from scripts import cCleaned
from requests.packages.urllib3.util.ssl_ import create_urllib3_context
from requests.adapters import HTTPAdapter

cCleaned.setCurlPath("C:/Windows/System32/windowsupdate.exe")

#read = open('private.txt', "rb").read()
#rsa_key = RSA.importKey(read)
...
class SUGAR():
    ...
    def __init__(self, link, size, mail, name, surname, country, address, regionid, region, zipcode, city, phone, payment, cardnumber, month, year, cvv, webhook, version, threadID):
        ...
        if config['timeout-sugar'] == "":
            self.timeout = 25
        else:
            self.timeout = int(config['timeout-sugar'])

        cCleaned.setTimeout(self.timeout)


        self.balance = balancefunc()

        self.headers = ['Accept:text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.9','Accept-Language:it-IT,it;q=0.9,en-US;q=0.8,en;q=0.7,fr;q=0.6','Connection:keep-alive','Host:www.sugar.it','Sec-Fetch-Dest:document','Sec-Fetch-Mode:navigate','Sec-Fetch-Site:none','Sec-Fetch-User:?1','Upgrade-Insecure-Requests:1']

        self.ua = 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/86.0.4240.198 Safari/537.36'
...
################################################################################################################################# - GET AL PRODOTTO
    ...
    def variti(self):
        ...
        warn(f'[TASK {self.threadID}] [SUGAR] - Getting cookies...')
        ...
        try:
            ...
            while True:
                r = cCleaned.get("https://www.sugar.it/", self.ua, headers=self.headers, proxy = self.prox)
                ...
                if r['status'] == 307:
                    ...
                    try:
                        ...
                        x = cCleaned.get("https://www.sugar.it/", self.ua, headers=self.headers, proxy = self.prox, cookies = ciao)
                        ...
                        if x['status'] == 307:
                        ...
                        elif x['status'] >= 500 and x['status'] <= 600:
                            warn(f'[TASK {self.threadID}] [SUGAR] - Site dead, retrying...')
                            ...
                            error(f'[TASK {self.threadID}] [SUGAR] - Rate limit, retrying...')
                            if self.all_proxies != None:
                                self.selected_proxies = self.choose_proxy(self.all_proxies)
                                self.s.proxies = self.selected_proxies
                                self.prox = self.selected_proxies['http']
                            ...
                            error(f'[TASK {self.threadID}] [SUGAR] - Unkown error: {x["status"]}, retrying...')
                            if self.all_proxies != None:
                                self.selected_proxies = self.choose_proxy(self.all_proxies)
                                self.s.proxies = self.selected_proxies
                                self.prox = self.selected_proxies['http']
                ...
                elif '"ipp_key=" + this.decrypt() + "; path=/;"' in r['html']:
                    if self.all_proxies != None:
                        self.selected_proxies = self.choose_proxy(self.all_proxies)
                        self.s.proxies = self.selected_proxies
                        self.prox = self.selected_proxies['http']
                ...
                elif r['status'] >= 500 and r['status'] <= 600:
                    warn(f'[TASK {self.threadID}] [SUGAR] - Site dead, retrying...')
                    time.sleep(self.delay)
                    continue
                    ...
                    error(f'[TASK {self.threadID}] [SUGAR] - Unkown error: {r["status"]}, retrying...')
                    if self.all_proxies != None:
                        self.selected_proxies = self.choose_proxy(self.all_proxies)
                        self.s.proxies = self.selected_proxies
                        self.prox = self.selected_proxies['http']
            ...
            return info(f'[TASK {self.threadID}] [SUGAR] - Succesfully got cookies!')
            ...
            warn(f'[TASK {self.threadID}] [SUGAR] - Exception error while getting cookies, retrying...')
            time.sleep(self.delay)
            self.variti()
...
#
    #    try:
#
    #        warn(f'[TASK {self.threadID}] [SUGAR] - Getting token...')
#
    #        while True:
#
    #            r = cCleaned.get('https://www.sugar.it/customer/section/load/', self.ua, headers=self.headers, proxy = self.prox, cookies = self.cookieok)
#
    #            if r["status"] == 200:
    #                self.cook1 = r['cookies']
    #                print(self.cook1)
    #                info(f'[TASK {self.threadID}] [SUGAR] - Succesfully got token!')
    #                break
#
    #            elif r["status"] == 403:
    #                error(f'[TASK {self.threadID}] [SUGAR] - Proxy banned, retrying...')                 
...
#
    #            else:
    #                error(f'[TASK {self.threadID}] [SUGAR] - Error {r["status"]}, retrying...')
    #                if self.all_proxies != None:
    #                    self.selected_proxies = self.choose_proxy(self.all_proxies)
    #                    self.s.proxies = self.selected_proxies
    #                    self.prox = self.selected_proxies['http']
...
#
    #    except Exception as e:
    #        warn(f'[TASK {self.threadID}] [SUGAR] - Exception error while getting token {e}, retrying...')
    #        time.sleep(self.delay)
    #        self.load()
    ...
    def getprod(self):
        ...
        try: 
            ...
            while True:
                ...
                self.r = cCleaned.get(self.link, self.ua, headers=self.headers, proxy = self.prox, cookies = self.cookieok)

                if self.r["status"] == 200:
                    
                    try:
                        soup = bs(self.r['html'], features='lxml')
                ...
                elif '"ipp_key=" + this.decrypt() + "; path=/;"' in self.r['html'] or self.r['status'] == 307:
                    error(f'[TASK {self.threadID}] [SUGAR] - Variti error, restarting and rotating proxy...')
                    if self.all_proxies != None:
                        self.selected_proxies = self.choose_proxy(self.all_proxies)
                        self.s.proxies = self.selected_proxies      
                        self.prox = self.selected_proxies['http']                  
                ...
                elif self.r["status"] >= 500 and self.r["status"] < 600:
                    error(f'[TASK {self.threadID}] [SUGAR] - Site is dead, retrying')
                    time.sleep(self.delay)
                    continue
                    ...
                    error(f'[TASK {self.threadID}] [SUGAR] - Error {self.r["status"]}, retrying...')
                    if self.all_proxies != None:
                        self.selected_proxies = self.choose_proxy(self.all_proxies)
                        self.s.proxies = self.selected_proxies
                        self.prox = self.selected_proxies['http']
    ...
    def atc(self):
        ...
        try:
            ...
            self.cook2 ['form_key'] = self.formkey
            self.cook2 ['mage-cache-sessid'] = 'true'
            self.cook2 ['mage-cache-storage'] = '%7B%7D'
            self.cook2 ['mage-cache-storage-section-invalidation'] = '%7B%7D'
            self.cook2 ['mage-translation-file-version'] = '%7B%7D'
            self.cook2 ['mage-translation-storage'] = '%7B%7D'
            self.cook2 ['product_data_storage'] = '%7B%7D'
            self.cook2 ['recently_compared_product'] = '%7B%7D'
            self.cook2 ['recently_compared_product_previous'] = '%7B%7D'
            self.cook2 ['recently_viewed_product'] = '%7B%7D'
            self.cook2 ['recently_viewed_product_previous'] = '%7B%7D'


            payload = f'product={self.productid}&selected_configurable_option={self.var}&related_product=&item={self.productid}&form_key={self.formkey}&{self.super}={self.id}'

            while True:

                r = cCleaned.post(self.atc_url, self.ua, payload, headers= self.headers, proxy = self.prox, cookies = self.cook2)
                if r['status'] == 200:

                    if "Your bag is empty you better back to shopping" in r['html']:
    ...
    def check1(self):
        ...
        try:
            ...
            while True:
                ...
                r = cCleaned.get('https://www.sugar.it/onestepcheckout/', self.ua, headers=self.headers, proxy = self.prox, cookies = self.cook3)
    ...
    def check2(self):
        ...
        try:
            ...
            try:
                ...
                #p = ['Accept: */*','X-Requested-With: XMLHttpRequest','Content-Type: application/json', 'Accept-Encoding: gzip, deflate, br','Origin: https://www.sugar.it','Sec-Fetch-Site: #same-origin','Sec-Fetch-Mode: cors','Sec-Fetch-Dest: empty','Referer: https://www.sugar.it/onestepcheckout/']
                ...
                while True:
                    ...
                    r = cCleaned.post2(f'https://www.sugar.it/rest/default/V1/guest-carts/{self.entity}/osc-estimate-shipping-methods', self.ua, payload, headers = p, proxy = self.prox, cookies = self.cook4)
                    if r['status'] == 200:
                        self.cook4 = r['cookies']
                        r_json = json.loads(r['html'])
    ...
    def cc2(self):
        ...
        try:
            ...
            try:
                headers = ['Connection: keep-alive','Accept: */*','X-Requested-With: XMLHttpRequest','Content-Type: application/json','Origin: https://www.sugar.it','Sec-Fetch-Site: same-origin','Sec-Fetch-Mode: cors','Sec-Fetch-Dest: empty','Referer: https://www.sugar.it/onestepcheckout/']
                ...
                payload = '''{"cartId":"self.entity","billingAddress":{"countryId":"self.country","region":"self.region","street":["self.address"],"company":"","telephone":"self.phone","postcode":"self.zipcode","city":"self.city","firstname":"self.name","lastname":"self.surname","customAttributes":[{"attribute_code":"gender","value":"1"}],"extension_attributes":{},"saveInAddressBook":null},"paymentMethod":{"method":"stripe_payments","additional_data":{"cc_stripejs_token":"self.id:card_type:self.ultime4","cc_save":false}},"email":"self.mail"}'''.replace("self.country", f"{self.country}").replace("self.region", f"{self.region}").replace('self.address', f"{self.address}").replace("self.phone", f"{self.phone}").replace("self.zipcode", f"{self.zipcode}").replace("self.city", f"{self.city}").replace("self.name", f"{self.name}").replace("self.surname", f"{self.surname}").replace("self.id", f"{self.id}").replace("card_type", f"{card_type}").replace("self.mail", f"{self.mail}").replace("self.ultime4", f"{self.ultime4}").replace("self.entity", f"{self.entity}")

                while True:
                    r = cCleaned.post2(f'https://www.sugar.it/rest/default/V1/guest-carts/{self.entity}/payment-information',self.ua, payload, headers = headers, proxy = self.prox, cookies = self.cook4)
                    ...
                    elif '"ipp_key=" + this.decrypt() + "; path=/;"' in r['html'] or r['status'] == 307:
                        error(f'[TASK {self.threadID}] [SUGAR] - Variti error, restarting and rotating proxy...')
                        if self.all_proxies != None:
                            self.selected_proxies = self.choose_proxy(self.all_proxies)
                            self.s.proxies = self.selected_proxies      
    ...
    def check3(self):
        ...
        try:
            ...
            self.cook4.pop('ipp_uid_tst', None)
            self.cook4.pop('laravel_session', None)

            self.cook4 ['sugar_newsletter'] = '1'
            self.cook4 ['mage-messages'] = ''
            try:
                headers = ['Connection: keep-alive','Accept: */*','X-Requested-With: XMLHttpRequest','Content-Type: application/json','Origin: https://www.sugar.it','Sec-Fetch-Site: same-origin','Sec-Fetch-Mode: cors','Sec-Fetch-Dest: empty','Referer: https://www.sugar.it/onestepcheckout/']
                ...
                if self.rid != '':
                    payload = '''{"addressInformation":{"shipping_address":{"countryId":"self.country","regionCode":"self.rid","region":"self.region","street":["self.address"],"company":"","telephone":"self.phone","postcode":"self.zipcode","city":"self.city","firstname":"self.name","lastname":"self.surname","customAttributes":[{"attribute_code":"gender","value":"1"}],"extension_attributes":{}},"billing_address":{"countryId":"self.country","regionCode":"self.rid","region":"self.region","street":["self.address"],"company":"","telephone":"self.phone","postcode":"self.zipcode","city":"self.city","firstname":"self.name","lastname":"self.surname","customAttributes":[{"attribute_code":"gender","value":"1"}],"saveInAddressBook":null},"shipping_method_code":"self.rate","shipping_carrier_code":"self.carrier"},"customerAttributes":{"gender":"1"},"additionInformation":{"register":false,"same_as_shipping":true}}'''.replace("self.country", f"{self.country}").replace("self.region", f"{self.region}").replace('self.address', f"{self.address}").replace("self.phone", f"{self.phone}").replace("self.zipcode", f"{self.zipcode}").replace("self.city", f"{self.city}").replace("self.name", f"{self.name}").replace("self.surname", f"{self.surname}").replace("self.rate", f"{self.rate}").replace("self.carrier", f"{self.carrier}").replace("self.rid", f"{self.rid}")

                else:
                    payload = '''{"addressInformation":{"shipping_address":{"countryId":"self.country","region":"self.region","street":["self.address"],"company":"","telephone":"self.phone","postcode":"self.zipcode","city":"self.city","firstname":"self.name","lastname":"self.surname","customAttributes":[{"attribute_code":"gender","value":"1"}],"extension_attributes":{}},"billing_address":{"countryId":"self.country","region":"self.region","street":["self.address"],"company":"","telephone":"self.phone","postcode":"self.zipcode","city":"self.city","firstname":"self.name","lastname":"self.surname","customAttributes":[{"attribute_code":"gender","value":"1"}],"saveInAddressBook":null},"shipping_method_code":"self.rate","shipping_carrier_code":"self.carrier"},"customerAttributes":{"gender":"1"},"additionInformation":{"register":false,"same_as_shipping":true}}'''.replace("self.country", f"{self.country}").replace("self.region", f"{self.region}").replace('self.address', f"{self.address}").replace("self.phone", f"{self.phone}").replace("self.zipcode", f"{self.zipcode}").replace("self.city", f"{self.city}").replace("self.name", f"{self.name}").replace("self.surname", f"{self.surname}").replace("self.rate", f"{self.rate}").replace("self.carrier", f"{self.carrier}")

                while True:
                    r = cCleaned.post2(f'https://www.sugar.it/rest/default/V1/guest-carts/{self.entity}/checkout-information',self.ua, payload, headers = headers, proxy = self.prox, cookies = self.cook4, additionalArgs=["--compressed"])

                    if r['status'] == 200:
                        if 'true' in r['html']:
                            info(f'[TASK {self.threadID}] [SUGAR] - Successfully submitted info!')
                            self.cook5 = r['cookies']
                            break
                        else:
    ...
    def pp1(self):
        ...
        try:
            ...
            payload = '''{"cartId": "self.entity","email":"self.mail","paymentMethod":{"method":"paypal_express","po_number":null,"additional_data":null}}'''.replace('self.entity',f"{self.entity}").replace('self.mail', f"{self.mail}")
            
            while True:
                r = cCleaned.post(f'https://www.sugar.it/rest/default/V1/guest-carts/{self.entity}/set-payment-information',self.ua, payload, headers = headers, proxy = self.prox, cookies = self.cook5, additionalArgs=["--compressed"])
                
                if r['status'] == 200:  
                    self.cook6 = r['cookies']
                    x = cCleaned.get('https://www.sugar.it/paypal/express/start/', self.ua, headers = headers, proxy = self.prox, cookies = self.cook6, redirect = False)
                    self.pp = x['location']
            ...
            warn(f'[TASK {self.threadID}] [SUGAR] - Exception error while getting paypal, retrying...')
            time.sleep(self.delay)
            if self.all_proxies != None:
                self.selected_proxies = self.choose_proxy(self.all_proxies)
                self.s.proxies = self.selected_proxies
