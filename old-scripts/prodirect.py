...
class PRODIRECT():
    ...
    def __init__(self, name, surname, address1, address2, zipcode, city, region, country, phone, link, size, account, password, mode, webhook, version, threadID):
        ...
        try:
            if machineOS == "Darwin":
                path = os.path.dirname(__file__).rsplit('/', 1)[0]
                path = os.path.join(os.path.dirname(sys.argv[0]), 'prodirect/proxies.txt')
            elif machineOS == "Windows": 
                path = os.path.dirname(__file__).rsplit('\\', 1)[0]
                path = os.path.join(os.path.dirname(sys.argv[0]), "prodirect/proxies.txt")
            with open(f'{path}', 'r') as f:
                ...
                while True:
                    ...
                    if r.status_code == 200:
                        ...
                        try:
                            ...
                            size1 = soup.find("div", {"class":"select-holder float-inputs"})
                            ...
                            for option in options:
                                ...
                                if len(a) < 6:
                                    b = a.replace("½",".5")
                                    self.instock.append(b)
                            
                            if len(self.instock) >= 1:
                                info(f'[TASK {self.threadID}] [PRODIRECT] - Product in stock!')
                                info(f'[TASK {self.threadID}] [PRODIRECT] - {self.instock}')

                                if self.size =="RANDOM":
                                    self.sizechosen = random.choice(self.instock)
                                    warn(f'[TASK {self.threadID}] [PRODIRECT] - Adding to cart size {self.sizechosen}')
                                    break
                    
                                elif '-' in self.size:
                                    self.size1 = float(self.size.split('-')[0])
                                    self.size2 = float(self.size.split('-')[1])
                                    for x in self.instock:
                                        if self.size1 <= float(x) <= self.size2:
                                            self.sizerange.append(x)        
                                    self.sizechosen = random.choice(self.sizerange)
                                    warn(f'[TASK {self.threadID}] [PRODIRECT] - Adding to cart size {self.sizechosen}')
                                    break
                                    ...
                                    if self.size in self.instock:
                                        self.sizechosen = self.size
                                        warn(f'[TASK {self.threadID}] [PRODIRECT] - Adding to cart size {self.sizechosen}')
                                        break

                                    else:
                                        warn(f'[TASK {self.threadID}] [PRODIRECT] - {self.size} is not available, retrying...')
                                        continue

                            else:
                                warn(f'[TASK {self.threadID}] [PRODIRECT] - Out of stock, retrying...')
                                time.sleep(self.delay)
                                continue
        ...
        try:
            ...
            try:
                while True:
                    ...
                    r = self.s.post(self.link, proxies = self.selected_proxies, headers = self.headers, data = payload)

                    if r.status_code == 200:

                        if "successfully added to" in r.text:
