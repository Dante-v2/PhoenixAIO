...
class SNS():
    ...
    def __init__(self, row, webhook, version, threadID, DISCORD_ID):
        ...
        try:
            if machineOS == "Darwin":
                path = os.path.dirname(__file__).rsplit('/', 1)[0]
                path = os.path.join(os.path.dirname(sys.argv[0]), 'sns/proxies.txt')
            elif machineOS == "Windows": 
                path = os.path.dirname(__file__).rsplit('\\', 1)[0]
                path = os.path.join(os.path.dirname(sys.argv[0]), "sns/proxies.txt")
            with open(f'{path}', 'r') as f:
    ...
    def error(self, text):
        ...
        message = f'[TASK {self.threadID}] - [SNS] - {text}'
        error(message)
    ...
    def bar(self):
        if machineOS.lower() == 'windows':
            return ctypes.windll.kernel32.SetConsoleTitleW(
                f'Phoenix AIO {self.version} - SNS Running | 2cap Balance: {self.balance} | Checkout: {checkoutnum} | Carted: {carted} | Failed: {failed}')

        else:
            sys.stdout.write(f'\x1b]2;Phoenix AIO {self.version} - SNS Running | 2cap Balance: {self.balance} | Checkout: {checkoutnum} | Carted: {carted} | Failed: {failed}\x07')
    ...
    def solve_v3(self, url):
        if CAPTCHA == '2captcha':
            try:
                solver = TwoCaptcha(config['2captcha'])
                result = solver.recaptcha(sitekey='6LeKIW0UAAAAAA-ouoKHOnWuQDNymSwDFYeGP323', url=url)
                code = result['code']
                return code
            except Exception as e:
            ...
            try:
                ...
                payload = {
                    ...
                    "task": {
                        ...
                        "websiteURL": url,
                        "websiteKey":"6LeKIW0UAAAAAA-ouoKHOnWuQDNymSwDFYeGP323"
                    }
                }
            ...
            try:
                AUTO_SOLVE.send_token_request({
                    ...
                    "url"     : url, 
                    "siteKey" : "6LeKIW0UAAAAAA-ouoKHOnWuQDNymSwDFYeGP323", 
                    "version" : '1'
                })
                while True:
        ...
        while True:
            try:     
                ...
                if r.status_code == 200:
                    ...
                    view = container.find("div",{"class":"product-view"})
                    head = view.find("header",{"class":"product-view__header"})
                    title = head.find("h1",{"class":"product-view__title"}).text
                    self.title = title.replace("\n", " ")
                    price = head.find("div",{"class":"product-view__price"})
                    self.stock = price.find("span",{"class":"product-view__availability"}).text
                    self.stockx = self.stock.strip()
                    sazi = view.find("div",{"class":"product-view__wrapper"})
                    if self.stockx == 'InStock':
                        try:
                            lol = sazi.find("div", {"class":"product-view__no-shop"}).text
                            if len(lol) > 10:
                                self.warn(f'{self.title} not in stock, monitoring...')
                                time.sleep(self.delay)
                                continue
                        except:
                            if 'countdown release-date__countdown' not in self.text:
                                self.success(f'{self.title} in stock, getting sizes!')
                                try:
                                    self.zio = r.text.split('"/productimages/')[1].split('.png"')[0]
                                    self.sns = False
                                except:
                                    self.zio = r.text.split('productimage/')[1].split('.png')[0]
                                    self.sns = True
                                self.getimage()
                                self.price = price.find("span",{"class","price__current"}).text
                                prodform = soup.find("form",{"class":"product-form"})
                                daid = prodform.find("input",{"type":"hidden"})
                                self.did = daid["value"]
                                saizes = prodform.find("ul",{"class":"product-sizes"})
                                label = saizes.find_all("label",{"class":"product-sizes__label"})
                                self.sizes = []
                                self.id = []
                                for labels in label:
                                    labelz = labels.text
                                    a_string = labelz.rstrip("\n")
                                    x = a_string.replace("\n","")
                                    self.sizes.append(x)
                                inpit = saizes.find_all("input",{"name":"id"})
                                for value in inpit:
                                    values = value["value"]
                                    self.id.append(values)
                                tot = zip(self.sizes, self.id)
                                connect = list(tot)
                                self.sizerange = []
                                if self.size == "RANDOM":
                                ...
                                self.warn(f'{self.title} Not dropped yet...')
                                time.sleep(self.delay)
                                continue
                    elif self.stockx == 'OutOfStock':
                        self.warn(f'{self.title} OOS, retrying...')
                        time.sleep(self.delay)
                        continue
                    else:
                        self.build_proxy()
                        self.error('Unknown error parsing sizes, retrying...')
                        time.sleep(self.delay)
                        continue
    ...
    def getimage(self):
        ...
        while True:
            try:
                if self.sns:
                    r = self.s.get(
                        f'https://www.sneakersnstuff.com/snsproductimage/{self.zio}.png',
                        timeout = self.timeout
                    )
                else:
                    r = self.s.get(
        ...
        while True:
            try:
                if 'g-recaptcha' in self.text:
                    ...
                    r = self.s.post(
                        'https://www.sneakersnstuff.com/en/cart/add', 
                        data = payload, 
                        timeout = self.timeout
                    )
                    if r.status_code == 200:
                        ...
                        self.warn(f'Error 400: {r.status_code}, retrying...')
                        self.build_proxy()
                        self.recursive = False
                        break
                    elif r.status_code == 429:
                    ...
                    payload = {'did': f'{self.did}', 'id': f'{self.idreal}', 'partial': 'mini-cart'}
    ...
    def checkout1(self):
        headers = {
            'referer':'https://www.sneakersnstuff.com/en/cart/view'
        }
        self.s.headers.update(headers)
        ...
        while True:
            try:       
                r = self.s.get(
                    f"https://www.sneakersnstuff.com/customer/setregion?redirectUrl=%2Ffr%2Fcart%2Fview%3Fpartial%3Dajax-cart-view&countryCode={self.country}", 
                    timeout = self.timeout
                )
                if r.status_code == 200:
    ...
    def checkout2(self):
        ...
        while True:
            try:
                r = self.s.post(
                    "https://www.sneakersnstuff.com/auth/iscustomer", 
                    data = payload,
                    timeout = self.timeout
                )
                if r.status_code == 200:
    ...
    def discount(self):
        ...
        try:
            r = self.s.post(
                f"https://www.sneakersnstuff.com/en/cart/setcheckoutcode", 
                data = payload, 
                timeout = self.timeout
            )
            if r.status_code == 200:
    ...
    def checkout3(self):
        ...
        while True:
            try:            
                r = self.s.post(
                    'https://www.sneakersnstuff.com/en/cart/setpaymentmethod', 
                    data = payload, 
                    timeout = self.timeout
                )
                if r.status_code == 200:
                    if 'Your shopping bag is empty' in r.text:
                        ...
                        soup = bs(r.text, features='lxml')
                        self.shippingmethod = soup.find('div',{'class':'cart-section'}).find('div',{'class':'cart-section__content'}).find_all('li')[1].find('input',{'class':'shipping method'})['value']
                        break
                elif r.status_code == 429:
                    self.s.cookies.clear()
                    self.build_proxy()
    ...
    def pp(self):
        ...
        payload = {
            ...
            'toggle-billing-address': 'true',
            'billingfirstName':self.name,
            'billinglastName':self.surname,
            'billingaddressLine2':self.address,
            'billingaddressLine3':self.address2,
            'billinghouseNumber':'NA',
            'billingpostalCode':self.zipcode,
            'billingcity':self.city,
            'billingCountry':self.country,
            'billingphoneNumber':self.phone,
            'shipping-method':self.shippingmethod,
            'txvariant':'card',
            'payment-method':'54',
            'termsAccepted':'true'
        }
        while True:
            try:         
                r = self.s.post(
                    "https://www.sneakersnstuff.com/en/cart/process", 
                    data = payload, 
            ...
            try:            
                ...
                if r.status_code == 200:
                    if 'Your shopping bag is empty' in r.text:
                        ...
                        self.Monitor()
                        break
                    elif r.url == "https://www.sneakersnstuff.com/en/cart/view":   
                        self.declined()
                        sys.exit()
                        break
                    else:
        ...
        try:
            ...
            if len(self.token) > 1999:
                ...
                with open(path,'a',newline='') as f:
                    ...
                    writer.writerow({'SITE':'SNS','SIZE':f'{self.sizeinstock}','PAYLINK':f'{self.token}','PRODUCT':f'{self.title}'})
            else:
                ...
                with open(path,'a',newline='') as f:
                    ...
                    if len(self.sizeinstock) > 2:
                        self.sizeinstock = f'{self.sizeinstock[:2]}+'
                    writer = csv.DictWriter(f, fieldnames=fieldnames)
                    writer.writerow({'SITE':'SNS','SIZE':f'{self.sizeinstock}','PAYLINK':f'{self.token}','PRODUCT':str(f'{self.title}')})
            if self.payment == "CC":
                self.webhookcard()
            else:
                self.SuccessPP()
    ...
    def passcookies2(self):
        try:
            ...
            cookieStr = urllib.parse.quote(base64.b64encode(bytes(cookies, 'utf-8')).decode())
            if not cookieStr: return
            url = urllib.parse.quote(base64.b64encode(bytes("https://www.sneakersnstuff.com/", 'utf-8')).decode())
            self.token = f"https://api.phoenixbot.io/exploits/?cookie={cookieStr}&redirect={url}"     
            self.token2 = f"https://api.phoenixbot.io/mobile/?cookie={cookieStr}&redirect={url}"
            apiurl2 = "http://tinyurl.com/api-create.php?url="
            tinyasdurl2 = urllib.request.urlopen(apiurl2 + self.token2).read()
    ...
    def webhook(self):
        ...
        embed.add_embed_field(name='**SNS**', value = f'{self.title}', inline = True)
    ...
    def Pubblic_Webhook(self):
        ...
        embed.add_embed_field(name='**SNS**', value = f'{self.title}', inline = False)
    ...
    def SuccessPP(self):
        ...
        embed.add_embed_field(name=f'**SNS**', value = self.title, inline = False)
