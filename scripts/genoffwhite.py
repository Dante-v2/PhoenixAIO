...
class GENOFFWHITE():
    ...
    def __init__(self, row, webhook, version, i, DISCORD_ID):
        ...
        try:
            if machineOS == "Darwin":
                path = os.path.dirname(__file__).rsplit('/', 1)[0]
                path = os.path.join(os.path.dirname(sys.argv[0]), 'genoffwhite/proxies.txt')
            elif machineOS == "Windows": 
                path = os.path.dirname(__file__).rsplit('\\', 1)[0]
                path = os.path.join(os.path.dirname(sys.argv[0]), "genoffwhite/proxies.txt")
            with open(f'{path}', 'r') as f:
    ...
    def injection(self, session, response):
        ...
        try:
            ...
            or session.is_BFM_Challenge(response):
                warn(f'[TASK {self.threadID}] [GEN_OFF] - Solving Cloudflare v2 api2')
...
#
        ...
        #        "vatNumber": ""
        #    }
        #    response = self.s.post('https://www.off---white.com/api/legacy/v1/account/register', json = data)
        #    if response.status_code == 200:
        #        info(f'[TASK {self.threadID}] [GEN_OFF] - Account created!')
        #        del self.s.headers['ff-country']
        #        del self.s.headers['ff-currency']
        ...
        #        r = self.s.post('https://www.off---white.com/en-it/api/addressbook', json = data2)
        #        if r.status_code == 200:
        #            js = json.loads(r.text)
        #            idd = js['id']
        #            self.s.put(f'https://www.off---white.com/en-it/api/addressbook/shipping/{idd}')
        #            if r.status_code == 200:
        #                self.s.put(f'https://www.off---white.com/en-it/api/addressbook/billing/{idd}')
        #                info(f'[TASK {self.threadID}] [GEN_OFF] - Address Saved!')
        #                self.SavingAccount()
        #    else:
...
#
    ...
    #    try:
    #        path = os.path.dirname(__file__).rsplit('/', 1)[0]
    #        path = os.path.join(os.path.dirname(sys.argv[0]), "genoffwhite")
    ...
    def drops(self):
        ...
        self.s.headers.update(headers)
        r = self.s.get(f'https://drops.off---white.com/products/{self.surname}?subfolder=en-it')
        if r.status_code == 200:
            self.title = r.text.split('<title data-react-helmet="true">')
            self.csvsave()
    ...
    def csvsave(self):
        ...
        with open(file_to_open,'a',newline='') as f:
            ...
            writer.writerow({'SKU':self.surname,'TITLE':f'{self.title}'})
        cnum = cnum + 1
        self.bar()
        sys.exit(1)