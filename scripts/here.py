...
class HERE():
    ...
    def __init__(self, row, webhook, version, i, DISCORD_ID):
        ...
        #self.listsuccess = ['https://discord.com/api/webhooks/848225765370757150/jhbxqUfu3aS6pDDOVa6orSVdFQOB0kRwIfF-2MfGD_Mp52Vb3lfmpmzqWRua98YnxGt8','https://discord.com/api/webhooks/848225771461017610/LJaPYN-wMoa9eV3ev8Zeu0djxepdzjL4hleiPlH-tjTbyUJafWNBgSWaFKB_hsS4UI9x','https://discord.com/api/webhooks/848225774330052629/JqWRPzP3b6SxUTel0YdP6yWGqI5hT6Nehv-YTs21kABUAxbjERq8eYydKou3Ay0rMqdZ']
        ...
        if self.country == 'IT':
            ...
            self.cart = 'carrello'
            self.checklink = 'https://www.here-store.com/fatturazione'
        ...
        while True:
            try:
                ...
                if r.status_code == 200:
                    ...
                    self.build_proxy()
                    continue
            except (requests.exceptions.ProxyError, requests.exceptions.Connection, requests.exceptions.ConnectTimeout, requests.exceptions.ReadTimeout, requests.exceptions.ConnectionError):
        ...
        while True:
            try:
                headers = {
                    ...
                    'referer': f'https://www.here-store.com/{self.sku2}.html'
                }
                payload = {
                    'Quantity': '1',
                    'sizeTable': '',
    ...
    def appdiscountprep(self):
        ...
        try:
            ...
            if "stata applicata" in r.text:
                return self.success('Session almost ready, proceding...')
            ...
            try:
                ...
                if "geo.captcha" in r.text:
                    ...
                    if "t':'fe" in r.text:
                        ...
                        time.sleep(self.delay)
                        continue
                if r.status_code == 200 and 'b-cart__subtitle">1' in r.text or 'qty-value">1' in r.text:
                    ...
                    if self.restockmode:
                        try:
                            ...
                            self.img = f'https://www.here-store.com/dw/image/v2/BCLG_PRD/on/demandware.static/-/Sites-awlab-master-catalog/default/dwe3c277df/images/large/{color}_0.jpg?sw=300'
                            break
                        except:
        ...
        try:
            ...
            if "geo.captcha" in r.text:
                ...
                if "t':'fe" in r.text:
                    ...
                    self.appdiscount()
            if "stata applicata" in r.text:
                return self.success('Discount Submitted, proceding...')
            elif "se ha aplicado" in r.text:
                return self.success('Discount Submitted, proceding...')
            elif "has been applied" in r.text:
                return self.success('Discount Submitted, proceding...')
            ...
            try:
                ...
                if "geo.captcha" in r.text:
                    ...
                    if "t':'fe" in r.text:
                        ...
                        time.sleep(self.delay)
                        continue
                if 'Alcuni dati di pagamento non sono validi' in r.text:
                    self.error('Failed submitting payment, something is wrong in your csv...')
                    failed = failed + 1
                    self.bar()
                    self.stop = True
                    break
                if "ordine" in r.url.lower():
                    try:
                        soup = bs(r.text, features='lxml') 
                    ...
                    self.build_proxy()
                    continue
                elif "riepilogoordine" in r.url.lower():
                    try:
                        soup = bs(r.text, features='lxml') 
                        numero = soup.find("span",{"class":"b-checkout-confirmation__order-number"}).text
                        self.numero = numero.split('Numero ordine: ')[1]
                        ...
                        self.payment = 'PayPal' 
                        continue
                elif "token=ec" in r.url.lower():
                    self.success('Successfully checked out')
                    checkoutnum = checkoutnum + 1
                    self.bar()
                    xx = r.url.split("token=EC")[1]
                    pp = xx.split('&')[0]
        ...
        try:
            ...
            if len(self.token) > 1999:
                ...
                with open(path,'a',newline='') as f:
                    ...
                    writer.writerow({'SITE':'HERE','SIZE':f'{self.sizegiusta}','PAYLINK':f'{self.token}','PRODUCT':f'{self.sku}'})
            else:
    ...
    def SuccessCAD(self):
        try:
            ...
            embed.add_embed_field(name=f'||HERE {self.country}||', value = f'{self.sku}', inline = True)
